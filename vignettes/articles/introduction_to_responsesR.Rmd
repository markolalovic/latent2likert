...
This article introduces responsesR, an R package designed to create synthetic datasets that mimic the kind of survey data commonly analyzed in applied social research, particularly ordered categorical items, also known as Likert items. Likert items represent responses to survey questions or ratings, typically organized on a psychometric scale defined by verbal labels with roughly equidistant categories symmetrically centered around a neutral midpoint. The most common form has five categories: Strongly disagree, Disagree, Neutral, Agree, and Strongly agree.

The package simplifies the conversion of continuous random variables into simulated Likert items. Users can leverage continuous variables with known means and covariance structure, which undergo discretization into discrete variables. The discrete variables mirror the original continuous data through Lloyd's algorithm, a technique commonly utilized in signal processing and closely linked to k-means clustering. Furthermore, asymmetry can be introduced by incorporating skew normal distribution. Additionally, the package enables the reconstruction of latent parameters from discrete probability distributions, thereby enabling users to replicate existing survey data more accurately.
