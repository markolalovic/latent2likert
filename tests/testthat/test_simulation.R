...
testthat::test_that("simulate_likert using 5 levels and N(0,1) gives expected result", {
    n_levels <- 5
...
testthat::test_that("generate_corr_matrix returns a matrix that resembles a correlation matrix", {
  n_items <- 3
  ...
  for (i in seq_along(corr_inputs)) {
    ...
    testthat::expect_true(all(diag(res) == 1), info = paste("Failed on input:", i))
    testthat::expect_true(isSymmetric(res), info = paste("Failed on input:", i))
  }
})
