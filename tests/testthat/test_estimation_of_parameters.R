...
test_that("`mu` and `sd` are well estimated given `pk` for normal case", {
...
test_that("`mu` and `sd` are well estimated given `pk` for skew case", {
...
test_that("`mu` and `sd` are well estimated given data for normal case", {
...
test_that("`mu` and `sd` are well estimated given data for skew case", {
  params <- c("mu"=0.5, "sd"=0.5) # actual parameters
  n <- 1000 # number of observations
  gamma1 <- 0.5 # skewness
...
test_that("`mu` and `sd` are well estimated given data for multivariate case", {
  # actual parameters
  mu <- c(0.6, -0.1, 0.3)
  sd <- c(1.3, 0.5, 1.5)
...
test_that("parameters `mu` and `sd` are well estimated in hard random case", {
  ...
  estimates <- estimate_mu_sd(pk, K, cp[["gamma1"]], trace = TRUE)
  actual <- c(cp[["mu"]], cp[["sd"]])
  expect_equal(estimates, actual, tolerance=0.1)
})

test_that("missing K raises error", {
  pk <- list("1"=0.313, "2"=0.579, "3"=0.105, "4"=0.003)
