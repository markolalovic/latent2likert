...
#' @param n_levels: Integer or vector of integers. The number of response categories 
#'              for each Likert scale item. Defaults to 5.
...
#'           "random" for a random correlation matrix, or an actual correlation matrix. 
#'           Defaults to 0.
rLikert <- function(size, n_levels, n_items, 
  ...
  if (corr_case == 1) {
    ...
    for (i in seq_len(n_items)) {
      data[, i] <- generate_responses(size, n_levels[i], 
                                      mean[i], sd[i], skew[i])
    }
  ...
  for (i in seq_len(n_items)) {
    ...
    data[, i] <- findInterval(latent_variables[, i], c(-Inf, endp, Inf))
  }
  colnames(data) <- paste0("Y", seq_len(n_items))
  return(data)
}
...
calc_endpoints = function(n_levels, skew) {
    ...
    endp = discretize_density(densityFn, n_levels)[["endp"]]
    return(endp)
}

# Returns a vector of probabilities across response levels
calc_Likert_prob = function(n_levels, cp) {
    dp = convert_params(cp)
    densityFn = function(x) {
        density_sn(x, dp[["xi"]], dp[["omega"]], dp[["alpha"]])
    }
    endp = calc_endpoints(n_levels, cp[["skew"]])
    prob = calc_probs(densityFn, endp)
    return(prob)
}
...
handle_corr_case <- function(corr) {
  ...
  if (is.character(corr) && corr == "random") return(2)
  if (is.matrix(corr)) return(4)
